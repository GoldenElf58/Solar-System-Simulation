    ...
    def limit(self, magnitude_limit: float, angle_limit: float = None) -> 'PolarVector':
        if angle_limit is not None:
            return PolarVector(max(min(self.angle, angle_limit), -angle_limit),
                               max(min(self.magnitude, magnitude_limit), -magnitude_limit)).limit(angle_limit)
        return PolarVector(self.angle, max(min(self.magnitude, magnitude_limit), -magnitude_limit))
